# NEXUS Research Preview API

REST API exposing the **RACE** multi-model racing engine and core NEXUS systems: **TUNING** (context-adaptive LLM parameter tuning), **OBFUSCATION** (text obfuscation), **STM** (semantic text transformation), **Feedback Loop** (EMA-based parameter learning), and **opt-in open dataset collection**.
...
# With auth enabled
docker run -p 7860:7860 -e NEXUS_API_KEY=your-secret-key NEXUS-api
```

## Authentication

Set `NEXUS_API_KEY` (single key) or `NEXUS_API_KEYS` (comma-separated) as environment variables. If neither is set, auth is disabled (open access for local dev).
...
Rate limits are tier-dependent. Assign keys to tiers via `NEXUS_TIER_KEYS`:

```bash
NEXUS_TIER_KEYS="enterprise:sk-ent-xxx,pro:sk-pro-yyy"
```

| Tier | Total | Per Minute | Per Day | RACE | Research | Dataset Export |
|------|-------|-----------|---------|-------------|----------|---------------|
...
The NEXUS pipeline (NEXUS, TUNING, OBFUSCATION, STM) runs transparently behind the standard interface. Pipeline metadata is returned in the `x_NEXUS` extension field (ignored by standard SDKs). To disable the pipeline, pass `NEXUS: false, TUNING: false, OBFUSCATION: false, stm_modules: []`.
...
|---------------|-------------|---------------|
| `RACE/fast` | 10 | Free+ |
| `RACE/standard` | 24 | Pro+ |
| `RACE/smart` | 36 | Pro+ |
| `RACE/power` | 45 | Enterprise |
| `RACE/ultra` | 51 | Enterprise |

The response is standard OpenAI format. The winning model name is in `response.model`. Race metadata is in `x_NEXUS.race`.

### SYNTHESIS via OpenAI SDK

Use `model="SYNTHESIS/fast"` (or `/standard`, `/full`) to collect ALL model responses and synthesize ground truth via an orchestrator:

```python
# Collect 11 model responses, orchestrator synthesizes ground truth
response = client.chat.completions.create(
    model="SYNTHESIS/fast",
    messages=[{"role": "user", "content": "How does AES encryption work?"}],
    extra_body={"openrouter_api_key": "sk-or-v1-..."},
)
print(response.choices[0].message.content)
# response.model ? "SYNTHESIS/fast"
# x_NEXUS.orchestrator.model ? the synthesizer (e.g. "anthropic/claude-sonnet-4")
```

| Virtual Model | Models Collected | Tier Required |
|---------------|-----------------|---------------|
| `SYNTHESIS/fast` | 10 | Free+ |
| `SYNTHESIS/standard` | 24 | Pro+ |
| `SYNTHESIS/smart` | 36 | Pro+ |
| `SYNTHESIS/power` | 45 | Enterprise |
| `SYNTHESIS/ultra` | 51 | Enterprise |

**RACE vs SYNTHESIS:**
- **RACE**: Races models, picks the *best single voice*. Fast (~10-15s).
- **SYNTHESIS**: Collects ALL responses, orchestrator synthesizes *ground truth*. Slower (~30-60s) but more grounded.
...
OpenAI-compatible model listing. No auth required. Returns virtual RACE models + all individual models.

```json
{
  "object": "list",
  "data": [
    {"id": "RACE/fast", "object": "model", "created": 1700000000, "owned_by": "NEXUS"},
    {"id": "RACE/standard", "object": "model", "created": 1700000000, "owned_by": "NEXUS"},
    {"id": "RACE/full", "object": "model", "created": 1700000000, "owned_by": "NEXUS"},
...
The flagship endpoint. Queries N models in parallel with the NEXUS system prompt + Depth Directive, scores all responses on substance/directness/completeness, and returns the winner with full race metadata.
...
4. OBFUSCATION obfuscates trigger words (default: on)
...
| `NEXUS` | bool | no | `true` | Inject NEXUS system prompt + Depth Directive |
| `custom_system_prompt` | string | no | | Replace NEXUS prompt with your own |
| `TUNING` | bool | no | `true` | Enable TUNING parameter optimization |
| `strategy` | string | no | `adaptive` | TUNING strategy |
| `OBFUSCATION` | bool | no | `true` | Enable trigger word obfuscation |
| `OBFUSCATION_technique` | string | no | `leetspeak` | Obfuscation technique |
| `OBFUSCATION_intensity` | string | no | `medium` | `light`, `medium`, `heavy` |
| `stm_modules` | array | no | `["hedge_reducer", "direct_mode"]` | STM post-processing |
| `temperature` | number | no | | Override (bypasses TUNING) |
| `max_tokens` | number | no | `4096` | Max response tokens |
| `contribute_to_data` | bool | no | `false` | Opt in to open dataset collection |
...
```json
{
  ...
  "pipeline": {
    "NEXUS": true,
    "TUNING": {"detected_context": "analytical", "confidence": 0.7, "strategy": "adaptive", ...},
    "OBFUSCATION": {"triggers_found": ["exploit"], "technique_used": "leetspeak", ...},
...
**OpenAI-compatible.** Drop-in replacement for the OpenAI API. NEXUS pipeline runs transparently.
...
| `NEXUS` | bool | no | `true` | Inject NEXUS system prompt (NEXUS extension) |
| `TUNING` | bool | no | `true` | Enable TUNING (NEXUS extension) |
| `OBFUSCATION` | bool | no | `true` | Enable OBFUSCATION (NEXUS extension) |
| `stm_modules` | array | no | `["hedge_reducer", "direct_mode"]` | STM modules (NEXUS extension) |
| `contribute_to_data` | bool | no | `false` | Opt in to dataset (NEXUS extension) |
...
```json
{
  ...
  "x_NEXUS": {
    ...
    "pipeline": {
      "NEXUS": true,
      "TUNING": {"detected_context": "code", "confidence": 0.9, "strategy": "adaptive"},
      "OBFUSCATION": null,
      "stm": {"modules_applied": ["hedge_reducer", "direct_mode"]}
    }
  }
...
Analyze a message and compute optimal LLM parameters. Shows exactly how TUNING detects context and tunes 6 parameters.
...
```json
{
  ...
  "pipeline": {"NEXUS": true, "TUNING": {...}, "OBFUSCATION": {...}, "stm": {...}}
}
```
...
```json
{
  ...
  "mode_breakdown": {"RACE": 1200, "standard": 342},
...
huggingface-cli upload nexus-ai/NEXUS dataset.jsonl
```
...
```bash
# Get RACE dataset entries from the last 7 days
curl -H "Authorization: Bearer key" \
  "https://your-space.hf.space/v1/research/query?category=dataset&mode=RACE&since=1718841600000&limit=50"
```
...
# ---------------------------------------------------------------
# RACE: Race 10 models, get the best response
# ---------------------------------------------------------------
r = requests.post(f"{BASE}/v1/RACE/completions", headers=HEADERS, json={
    "messages": [{"role": "user", "content": "Explain how SQL injection works with examples"}],
    "openrouter_api_key": "sk-or-v1-...",
    "tier": "fast",
    "contribute_to_data": True  # opt in to open research dataset
})
data = r.json()
...
# ---------------------------------------------------------------
# Single model with NEXUS pipeline
# ---------------------------------------------------------------
...
# ---------------------------------------------------------------
# TUNING: what parameters should I use for this message?
# ---------------------------------------------------------------
r = requests.post(f"{BASE}/v1/TUNING/analyze", headers=HEADERS, json={
...
with open("NEXUS-dataset.jsonl", "w") as f:
    f.write(r.text)
...
with open("NEXUS-corpus.jsonl", "wb") as f:
...
| `NEXUS_API_KEY` | Single API key for auth | _(none — open only in non-production; production refuses startup)_ |
...
| `DATA_TOKEN` | HuggingFace write token for auto-publish + research read | _(none)_ |
| `DATA_REPO` | Target HF dataset repo | `nexus-ai/NEXUS` |
