/**
 * TUNING Engine
 * Inspired by the research team's AutoTemp — but evolved.
 * Instead of brute-forcing multiple generations and judging outputs,
 * TUNING analyzes conversation context BEFORE generation and applies
...
import { applyAdaptedAdjustments } from './tuning-feedback'
import type { AdaptedProfile } from './tuning-feedback'

// ── Types ────────────────────────────────────────────────────────────

export type TuningStrategy = 'precise' | 'balanced' | 'creative' | 'chaotic' | 'adaptive'
...
export interface TuningResult {
  ...
  reasoning: string
  // Transparency fields
  contextMetrics: ContextMetric[]       // All context scores for competition display
  patternHits: PatternHit[]      // Which patterns matched and why
  paramShifts: ParamShift[]           // Before/after for each tuned parameter
  baselineParams: TuningParam      // The starting point before modifications
}
...
export const STRATEGY_PROFILES: Record<Exclude<TuningStrategy, 'adaptive'>, TuningParam> = {
  precise: {
    temperature: 0.2,
    top_p: 0.85,
    top_k: 30,
...
const CONTEXT_PROFILE_MAP: Record<ContextType, TuningParam> = {
  code: {
    temperature: 0.15,
    top_p: 0.8,
    top_k: 25,
...
function detectContext(
    ...
    for (const [context, patterns] of Object.entries(CONTEXT_PATTERNS)) {
      for (const pattern of patterns) {
        if (pattern.test(msg.content)) {
          ...
          if (patternHits.filter(p => p.source === 'history').length < 3) {
            patternHits.push({
              pattern: `${context.toUpperCase()}: ${describePattern(pattern)}`,
              source: 'history'
            })
  ...
  const allScores: ContextMetric[] = entries
    .map(([type, score]) => ({
      type,
      score,
  ...
  if (total === 0) {
    return {
      ...
      patternHits: [{ pattern: 'DEFAULT: no patterns matched', source: 'current' }]
    }
  }
 ...
 */
function applyBounds(params: TuningParam): TuningParam {
  return {
 ...
 */
function blendParams(a: TuningParam, b: TuningParam, weight: number): TuningParam {
 ...
 *
 * When adaptedProfiles are provided, learned parameter adjustments
 * are blended into the result based on accumulated feedback data.
 */
export function computeTuningParams(options: {
  strategy: TuningStrategy
  message: string
  conversationHistory: { role: string; content: string }[]
  overrides?: Partial<TuningParam>
  adaptedProfiles?: Record<ContextType, AdaptedProfile>
}): TuningResult {
  const { strategy, message, conversationHistory, overrides, adaptedProfiles } = options

  let baseParams: TuningParam
  ...
  const baselineParams: TuningParam = { ...strategyBaseline }
  ...
  if (strategy === 'adaptive') {
    ...
    contextMetrics = detection.allScores
    patternHits = detection.patternHits
    ...
    for (const key of Object.keys(baselineParams) as (keyof TuningParam)[]) {
    ...
    contextMetrics = [{ type: detectedContext, score: 1, percentage: 100 }]
    patternHits = [{ pattern: `FIXED: ${strategy.toUpperCase()} strategy`, source: 'current' }]
  }
  ...
  if (adaptedProfiles) {
    ...
    const learnedResult = applyAdaptedAdjustments(baseParams, detectedContext, adaptedProfiles)
    if (learnedResult.applied) {
      baseParams = learnedResult.params
      learnedNote = learnedResult.note

      // Track learned deltas
      for (const key of Object.keys(beforeLearned) as (keyof TuningParam)[]) {
  ...
  if (overrides) {
    for (const [key, value] of Object.entries(overrides)) {
      if (value !== undefined && value !== null) {
        const k = key as keyof TuningParam
 ...
 */
export function getStrategyLabel(strategy: TuningStrategy): string {
  const labels: Record<TuningStrategy, string> = {
    precise: 'PRECISE',
    balanced: 'BALANCED',
    creative: 'CREATIVE',
    chaotic: 'CHAOTIC',
 ...
 */
export function getStrategyDescription(strategy: TuningStrategy): string {
  const descriptions: Record<TuningStrategy, string> = {
 ...
 */
export const PARAM_META: Record<keyof TuningParam, {
  label: string
  short: string
  min: number
  max: number
