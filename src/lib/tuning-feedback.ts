 ...
 *
 * The learned adjustments blend into TUNING's parameter selection — more feedback data
 * means more influence, capped at 50% weight so base profiles remain the foundation.
 */

import type { TuningParam, ContextType } from './tuning'
...
export interface AdaptedProfile {
  ...
  positiveParams: TuningParam   // EMA of params from upvoted responses
  negativeParams: TuningParam   // EMA of params from downvoted responses
  adjustments: Partial<TuningParam>  // computed delta to apply
  lastUpdated: number
}
...
const NEUTRAL_PARAMS: TuningParam = {
  temperature: 0.7,
  top_p: 0.9,
  top_k: 50,
 ...
 */
export function computeAdaptations(response: string): ResponseHeuristics {
...
export function createInitialLearningState(): LearningState {
  ...
  const adaptedProfiles: Record<string, AdaptedProfile> = {}
  ...
  return {
    history: [],
    adaptedProfiles: adaptedProfiles as Record<ContextType, AdaptedProfile>
  }
}
...
export function processLearning(
  ...
  const profile = { ...state.adaptedProfiles[record.contextType] }
  profile.sampleCount++
  ...
  const newProfiles = {
    ...state.adaptedProfiles,
    [record.contextType]: profile
  }

  return {
    history: newHistory,
    adaptedProfiles: newProfiles
  }
}
 ...
 */
function computeAdjustments(profile: AdaptedProfile): Partial<TuningParam> {
  ...
  const adj: Partial<TuningParam> = {}
  const keys: (keyof TuningParam)[] = [
 ...
 */
export function applyAdaptedAdjustments(
  baseParams: TuningParam,
  contextType: ContextType,
  adaptedProfiles: Record<ContextType, AdaptedProfile>
): { params: TuningParam; applied: boolean; note: string } {
  const profile = adaptedProfiles[contextType]
 ...
 */
export function getFeedbackStats(state: LearningState): {
  ...
  for (const ctx of contexts) {
    const profile = state.adaptedProfiles[ctx]
