...
/**
 * Auto-detect a self-hosted NEXUS API server at the same origin.
 *
 ...
 * is open (no NEXUS_API_KEY configured), proxy mode activates
...
export function useApiAutoDetect() {
  ...
  useEffect(() => {
    ...
    if (raceApiUrl && raceApiUrl !== 'http://localhost:7860' && raceApiKey) return
    ...
    async function detect() {
      try {
        ...
        // Step 2: Check if auth is open (no NEXUS_API_KEY required)
        ...
        if (tierRes.ok || tierRes.status === 429) {
          ...
          setRaceApiUrl(origin)
          if (!raceApiKey) {
            setRaceApiKey('self-hosted')
          }
          console.log('[NEXUS] Self-hosted API detected (open auth) at', origin)
          ...
          setRaceApiUrl(origin)
          console.log('[NEXUS] Self-hosted API detected (auth required) at', origin)
        }
      } catch {
  ...
  }, [isHydrated, apiKey, raceApiUrl, raceApiKey, setRaceApiUrl, setRaceApiKey])
}
