 ...
 * Research API Routes
 *
 * Read-access endpoints for the nexuslabs/NEXUS HuggingFace
...
import { getPublisherStatus } from '../lib/data-publisher'
...
researchRoutes.get('/info', (_req, res) => {
  res.json({
    name: 'NEXUS Research Dataset',
    repo: 'nexuslabs/NEXUS',
    repo_url: 'https://huggingface.co/datasets/nexuslabs/NEXUS',
    license: 'AGPL-3.0',
    description: 'Open research dataset for LLM robustness and safety evaluation. Contains opt-in interaction data with full pipeline metadata from RACE multi-model racing, TUNING context-adaptive parameter tuning, OBFUSCATION obfuscation, and STM output normalization.',
    ...
    schema: {
      dataset_entry: {
        ...
        response: 'string',
        TUNING: '{ strategy, detected_context, confidence, params, reasoning } | null',
        OBFUSCATION: '{ triggers_found, technique_used, transformations_count } | null',
        stm: '{ modules_applied: string[] } | null',
        RACE: '{ tier, models_queried, winner_model, all_scores, total_duration_ms } | null',
      ...
      metadata_event: {
        ...
        tier: 'string | null',
        stream: 'boolean',
        pipeline: '{ NEXUS, TUNING, OBFUSCATION, stm_modules, strategy }',
        TUNING: '{ detected_context, confidence } | null',
...
researchRoutes.get('/query', tierGate('research:full'), async (req, res) => {
  try {
    const query = {
      ...
      mode: req.query.mode as 'standard' | 'RACE' | undefined,
...
researchRoutes.get('/download', tierGate('corpus:download'), async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="NEXUS-corpus-${Date.now()}.jsonl"`)
