 ...
 *
 * In-memory rate limiter that reads limits from the request's AccessConfig
...
export function rateLimit(req: Request, res: Response, next: NextFunction): void {
  ...
  const accessConfig: AccessConfig | undefined = req.accessConfig
  const now = Date.now()

  // Resolve limits from tier config or fall back to env defaults
  const TOTAL_LIMIT = accessConfig?.rateLimit.total ?? parseInt(process.env.RATE_LIMIT_TOTAL || '5', 10)
  const MINUTE_LIMIT = accessConfig?.rateLimit.perMinute ?? parseInt(process.env.RATE_LIMIT_PER_MINUTE || '60', 10)
  const DAY_LIMIT = accessConfig?.rateLimit.perDay ?? parseInt(process.env.RATE_LIMIT_PER_DAY || '1000', 10)
  ...
  if (TOTAL_LIMIT > 0 && bucket.totalRequests >= TOTAL_LIMIT) {
    ...
    res.status(429).json({
      ...
      upgrade: tier === 'free' ? 'Set NEXUS_TIER_KEYS to assign a higher tier to your API key.' : undefined,
    })
    return
  }
