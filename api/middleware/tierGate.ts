 ...
 *   app.use('/v1/dataset',  apiKeyAuth, tierGate('dataset:export'), dataRoutes)
 */

import type { Request, Response, NextFunction } from 'express'
import type { Tier, AccessConfig } from '../lib/tiers'
import { getAccessConfig, TIER_CONFIGS } from '../lib/tiers'
...
type GateCheck =
  ...
  | 'metadata:events'
  | 'corpus:download'
  | `RACE:${'fast' | 'standard' | 'smart' | 'power' | 'ultra'}`

/**
 * Evaluate whether a tier config passes a given gate check.
 */
function passesGate(config: AccessConfig, gate: GateCheck): boolean {
  switch (gate) {
    case 'research:read':
    ...
    case 'RACE:fast':
      return config.raceTiers.includes('fast')
    case 'RACE:standard':
      return config.raceTiers.includes('standard')
    case 'RACE:smart':
      return config.raceTiers.includes('smart')
    case 'RACE:power':
      return config.raceTiers.includes('power')
    case 'RACE:ultra':
      return config.raceTiers.includes('ultra')
    default:
      return false
  }
}
...
export function tierGate(gate: GateCheck) {
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    const config = getAccessConfig(tier)
