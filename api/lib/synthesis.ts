 ...
 * SYNTHESIS Engine
 *
 * The hive-mind mode of NEXUS. Instead of racing models to pick a single winner
 * (RACE), SYNTHESIS collects ALL responses and feeds them to a strong
 * orchestrator model that synthesizes ground truth from the collective intelligence.
 *
 * Architecture:
 *   1. COLLECTION  — Query N models in parallel (reuses RACE racing infra)
 ...
 *
 * Key difference from RACE:
 *   RACE picks the BEST single voice.
 *   SYNTHESIS distills GROUND TRUTH from the crowd.
 */

import { queryModel, type ModelResult, type SpeedTier, getModelsForTier, scoreResponse } from './racing'
...
interface CollectionConfig {
  ...
  minResponses?: number
  /** Max time to wait for collection (default: 60000ms — longer than RACE) */
  hardTimeout?: number
...
export const SYNTHESIS_SYSTEM_PROMPT = `You are the SYNTHESIS ORCHESTRATOR — an impartial, analytical intelligence tasked with synthesizing ground truth from multiple AI model responses.
...
/**
 * Query all models and collect responses. Unlike RACE's race-and-exit,
 * SYNTHESIS waits for ALL models (up to the hard timeout) because every
...
export async function runSynthesis(
  ...
  // Phase 2: Score
  const scoredResponses: SynthesisResponse[] = rawResults.map(r => ({
    model: r.model,
    content: r.content,
