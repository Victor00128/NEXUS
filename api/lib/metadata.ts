 ...
 *
 * Always-on, privacy-first analytics for NEXUS.
 ...
 * - Pipeline flags (NEXUS, TUNING, OBFUSCATION, stm)
 * - TUNING detected context type + confidence
...
import { registerMetadataStore, checkMetadataThreshold } from './data-publisher'
...
export interface MetadataEvent {
  ...
  // Request shape
  endpoint: string
  mode: 'standard' | 'race' | 'race-failed' | 'race-error' | 'synthesis'
  tier?: string // fast | standard | full (RACE only)
  stream: boolean
  ...
  // TUNING metadata (context detection only, no content)
  tuning?: {
  ...
  // Dynamic Upgrade metadata (streaming upgrades)
  liquid?: {
...
export interface MetadataStats {
  ...
  // Pipeline usage
  pipeline: {
    NEXUS_rate: number // % of requests with NEXUS enabled
    TUNING_rate: number
    OBFUSCATION_rate: number
    stm_usage: Record<string, number> // module -> count
    strategy_usage: Record<string, number>
  }

  // Context detection (from TUNING)
...
export function getStats(): MetadataStats {
  ...
  if (total === 0) {
    return {
      ...
      pipeline: { NEXUS_rate: 0, TUNING_rate: 0, OBFUSCATION_rate: 0, stm_usage: {}, strategy_usage: {} },
      contexts: {},
  ...
  for (const e of events) {
    ...
    if (e.pipeline.nexus) NEXUSCount++
    if (e.pipeline.tuning) TUNINGCount++
    if (e.pipeline.obfuscation) OBFUSCATIONCount++
    if (e.pipeline.strategy) {
      strategyUsage[e.pipeline.strategy] = (strategyUsage[e.pipeline.strategy] || 0) + 1
    }
    for (const mod of e.pipeline.stmModules) {
      stmUsage[mod] = (stmUsage[mod] || 0) + 1
    }

    if (e.tuning?.detected_context) {
      const ctx = e.tuning.detected_context
  ...
  return {
    ...
    pipeline: {
      NEXUS_rate: Math.round((NEXUSCount / total) * 100) / 100,
      TUNING_rate: Math.round((TUNINGCount / total) * 100) / 100,
      OBFUSCATION_rate: Math.round((OBFUSCATIONCount / total) * 100) / 100,
      stm_usage: stmUsage,
