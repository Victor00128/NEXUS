/**
 * RACE Engine (API Edition)
 *
 * The flagship mode of NEXUS. Queries multiple LLMs in parallel via OpenRouter,
 * scores responses on substance/directness/completeness, and returns the winner.
 *
 * Pipeline: NEXUS prompt ? Depth Directive ? TUNING ? OBFUSCATION ?
 *           N models in parallel ? Score ? Pick winner ? STM post-process
 */

// -- NEXUS System Prompt (imported from single source of truth) -----
export { NEXUS_SYSTEM_PROMPT } from '../../src/lib/system-prompt'

// -- Depth Directive (appended to all RACE prompts) -----------
...
export const RACE_MODELS = {
  ...
  // ?? POWER TIER (+10 models = 49 cumulative): Frontier, cutting-edge
  power: [
...
// -- NEXUS Parameter Boost ------------------------------------------

/** Apply the NEXUS parameter boost (pushes toward more direct/uncensored output) */
export function applyNexusBoost(params: Record<string, number | undefined>): Record<string, number | undefined> {
  return {
    ...params,
