 ...
 *   DATA_REPO   — target repo (e.g. "nexuslabs/NEXUS")
 *
...
async function validateCredentials(): Promise<void> {
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.error(`[HF Publisher] ✗ AUTHENTICATION FAILED (${res.status}) — DATA_TOKEN is invalid or lacks write access to "${repo}"`)
      console.error(`[HF Publisher] ✗ Data will accumulate in memory but NEVER publish. Fix your DATA_TOKEN.`)
...
export function startPeriodicFlush() {
  if (!isPublisherEnabled()) {
    console.warn('[HF Publisher] Periodic flush SKIPPED — publisher not enabled (missing DATA_TOKEN or DATA_REPO)')
    return
  }
