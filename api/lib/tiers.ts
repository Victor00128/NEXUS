 ...
 * Keys are mapped to tiers via NEXUS_TIER_KEYS env var.
 *
 * Format:  NEXUS_TIER_KEYS="enterprise:sk-ent-xxx,pro:sk-pro-yyy,pro:sk-pro-zzz"
 *
 * Any key in NEXUS_API_KEY(S) that isn't in NEXUS_TIER_KEYS defaults to "free".
...
export interface AccessConfig {
  ...
  /** RACE tiers allowed */
  raceTiers: ('fast' | 'standard' | 'smart' | 'power' | 'ultra')[]
...
export const TIER_CONFIGS: Record<Tier, AccessConfig> = {
  free: {
    name: 'free',
    label: 'Free',
    rateLimit: {
  ...
  pro: {
    ...
    raceTiers: ['fast', 'standard', 'smart', 'power'],
    maxRaceModels: 36,
    researchAccess: 'read',
...
if (tierKeyMap.size > 0) {
  ...
  console.log(
    `[NEXUS] Tier keys loaded: ${counts.enterprise} enterprise, ${counts.pro} pro, ${counts.free} free`,
  )
}
 ...
 */
export function getAccessConfig(tier: Tier): AccessConfig {
